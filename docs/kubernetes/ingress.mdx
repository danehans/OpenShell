...
Create a shared Gateway in the controller namespace. `allowedRoutes.namespaces.from: All` lets the OpenShell `GRPCRoute` attach from its release namespace:
...
Use a different Gateway name if you create it in the `openshell` namespace. The default chart-generated name, `openshell`, collides with the existing OpenShell Service. For a dedicated development Gateway, set `grpcRoute.gateway.create=true`, `grpcRoute.gateway.className=agentgateway`, and `grpcRoute.gateway.name=openshell-ingress`.
